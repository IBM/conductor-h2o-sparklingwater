...
function checkSparkVersion() {
  ...
  if [ -z "$INSTALLED_SPARK_FULL_VERSION" ]; then
    echo "Cannot determine Spark version"
    exit 1
  fi
...
export MAJOR_VERSION=$(echo "$VERSION" | cut -d . -f 1,2)
export PATCH_VERSION=$(echo "$VERSION" | cut -d . -f 3)
...
export DEFAULT_DRIVER_MEMORY=4G
